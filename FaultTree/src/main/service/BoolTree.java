...
//        System.out.println(value.size());
    ...
    public double BooltimeTree(ArrayList<String> id, long time){
        ...
        if(time2.get(0).size()==0){
            ...
            for(int i=0;i<value1.get(0).size()-1;i++){
                if(value1.get(0).get(i)==1){
                    for(int j=i+1;j<value1.get(0).size();j++){
                        if(value1.get(0).get(j)==1){
                            if(isinterval1(time1.get(0).get(i),time1.get(0).get(j),time2.get(time2.size()-1))){
                                y++;
                            }else{
                                x++;
                            }
    ...
    //bool型专用方法  返回测量量为"1"的比例  table为表名，id为测点ID，time为后期时间
//    public double BooleanTree (String table,String id, long time) throws ClassNotFoundException, SQLException {
    public double BooleanTree1 (String id,long time,String val,int timeinter) throws ClassNotFoundException, SQLException, ParseException {
        DataBoolUtils data = post_request.queBool(id,time,timeinter);//获取boolean型数据
           /* DataUtils data = null;//获取boolean型数据
            try {
                data = (DataUtils) DaoTree.queBool(time,id);
            } catch (ParseException e) {
...
//        System.out.println(value.size());
    ...
    //判断命令类子节点所用的方法
    public double BooltimeTree1(ArrayList<String> id, long time,int timeinter){
        double p=0.0;
        int x = 0;
        int y = 0;
        ArrayList<ArrayList<Integer>> value1 = new ArrayList<>();
        ...
        for(int i=0;i<id.size();i++){
            ArrayList<Long> newtime = new ArrayList<>();
            DataBoolUtils data = post_request.queBool(id.get(i),time,timeinter);
            for(int j=0;j<data.getValue().size();j++){
                if(data.getValue().get(j)==1){
                    newtime.add(data.getTime().get(j));
                }
